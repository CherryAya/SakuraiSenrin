...
@pytest.mark.asyncio
async def test_self_unban_user_flow_prompts_and_finishes(
    app: App,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(
            return_value=SimpleNamespace(
                final_message="已解除全局黑名单。已使用 1/2 次自助解封，剩余 1 次。",
                should_retry=False,
            )
        ),
    )
    first = build_private_message_event("#self.unban user", user_id=10001)
    second = build_private_message_event(
        "之前连续刷屏已经停止，后续会遵守群规正常交流",
        user_id=10001,
        message_id=2,
    )
...
@pytest.mark.asyncio
async def test_self_unban_group_denies_normal_member(
    app: App,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    sys.modules.pop("src.plugins.self_unban", None)
    ...
    monkeypatch.setattr(
        command_module.self_unban_service,
        "prepare_group_request",
        AsyncMock(return_value="只有群主或管理员才能为本群发起自助解封。"),
    )
    event = build_group_message_event("#self.unban group", role="member")
...
async def test_self_unban_no_check_bypasses_runtime_blacklist_block(
    ...
    event = build_private_message_event("#self.unban user", user_id=10001)
