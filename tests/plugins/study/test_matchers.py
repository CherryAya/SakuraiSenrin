...
async def test_study_guided_forward_reply_prompts_import_mode(
    ...
    async with app.test_matcher(study_plugin.study_command) as ctx:
        ...
        fifth = build_group_message_event("", message_id=5)
        fifth.message = Message([MessageSegment.forward("54321")])
...
@pytest.mark.asyncio
async def test_study_guided_flow_parses_response_placeholders(
    app: App,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    result = WordbankAddResult(
        ...
        response_item_id=22,
        trigger_text="晚安",
        response_text="[@触发者] [戳触发者]",
        scope="all_groups",
        probability=1.0,
        weight=3,
        status="pending",
        trigger_shape=shape_from_text("晚安"),
        response_shape=shape_from_response_text("[@触发者] [戳触发者]"),
    )
    handle_guided = AsyncMock(return_value=result)
    build_result_message = AsyncMock(return_value=text_message("词条已提交审核"))
    record_submission = AsyncMock(return_value=None)
    ...
    async with app.test_matcher(study_plugin.study_command) as ctx:
        ...
        second = build_group_message_event("M", message_id=2)
        third = build_group_message_event("F", message_id=3)
        fourth = build_group_message_event("晚安", message_id=4)
        fifth = build_group_message_event("[@触发者] [戳触发者]", message_id=5)
        sixth = build_group_message_event("3", message_id=6)
    ...
    handle_guided.assert_awaited_once()
    guided_kwargs = handle_guided.await_args_list[0].kwargs
    assert guided_kwargs["response_shape"] == shape_from_response_text(
        "[@触发者] [戳触发者]"
    )
