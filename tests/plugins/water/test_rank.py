...
@pytest.mark.asyncio
async def test_group_report_image_supports_three_digit_group_rank_labels() -> None:
    avatar = BuildImage.new("RGBA", (128, 128), "#F6B7D2")
    data = WaterGroupReportImageData(
        title="Senrin水王日报",
        badge="",
        ...
        group_rank_title="群聊当日排名",
        group_rank_summary="本群当前排名 #128 / 512 · 较昨日 +12",
        group_rank_items=[
            WaterGroupDailyRankCardItem(
                group_id="20001",
                display_name="测试群",
    ...
    for subject, scopes in legal_scopes.items():
        for scope in scopes:
            for period in periods:
                ...
                assert "CQ:image" in str(render_message_plan_input(message)), (
                    subject,
                    scope,
                    period,
                )
